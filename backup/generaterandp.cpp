...
void generate_rand_sphere(particles *pt, par *par)
{
    ...
    for (int p = 0; p < 2; p++)
    {
        for (na = 0; na < nback_cube; ++na) // set particles in cube background
        {
            ...
            pt->pos1z[p][na] = pt->pos0z[p][na] = uniform_dist(gen) * (par->posH_15[2] - par->posL_15[2]) + par->posL_15[2];
            pt->q[p][na] = qs[p];
            // pt->m[p][na] = mp[p];
        }
        ...
        for (int n = nback; n < n_partd; n++)  // set particles in spot
        {
            ...
            pt->pos0x[p][n] = r * x;
            pt->pos1x[p][n] = pt->pos0x[p][n] + (normal_dist(gen) * sigma[p] + x * v0_r) * par->dt[p];
            pt->pos0y[p][n] = r * y;
            pt->pos1y[p][n] = pt->pos0y[p][n] + (normal_dist(gen) * sigma[p] + y * v0_r) * par->dt[p];
            pt->pos0z[p][n] = r * z;
            pt->pos1z[p][n] = pt->pos0z[p][n] + (normal_dist(gen) * sigma[p] + z * v0_r) * par->dt[p];
            pt->q[p][n] = qs[p];
